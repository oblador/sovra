// Code copied from [Oxc](https://github.com/oxc-project/oxc/blob/main/npm/oxc-parser/scripts/generate-packages.mjs)

import * as fs from "node:fs";
import { resolve } from "node:path";
import { fileURLToPath } from "node:url";

const SOVRA_ROOT = resolve(fileURLToPath(import.meta.url), "../..");
const PACKAGES_ROOT = resolve(SOVRA_ROOT, "..");
const BINARY_ROOT = resolve(SOVRA_ROOT, "../../napi");
const MANIFEST_PATH = resolve(SOVRA_ROOT, "package.json");

console.log("SOVRA_ROOT", SOVRA_ROOT);
...
function package_name(target) {
  return `@sovra/binding-${target}`;
}
function generateNativePackage(target) {
  const binaryName = `sovra.${target}.node`;

  const packageRoot = resolve(PACKAGES_ROOT, `sovra-${target}`);
